...
# all.analysis_unmod <- read.xlsx(path, "ALL_Mid",stringsAsFactors=T)
# avt.analysis_both <- read.xlsx(path, "AVT_NEW", stringsAsFactors=T)
# 
# all.analysis_withoutavt <- dataToNumeric(all.analysis_unmod)
# avt.analysis <- dataToNumeric(avt.analysis)
# 
# avt.analysis <- avt.analysis_both %>% 
#   filter(name == "AVGARCH")
# 
# all.analysis_test <- all.analysis_withoutavt %>%
#   filter(name != "AVGARCH") %>% 
#   rbind(avt.analysis)
# 
# all.analysis <- all.analysis_test
...
# extracts a given coefficient from a subset
coef_estimator_V1 <- function(all_fit_data, fit_data, arg, coeff){
  if(grepl(arg, "ARCH")){
    logic <- fit_data$name == arg
  } else if(is.null(arg)){
    logic <- rep(1, nrow(fit_data))
  } else{
    logic <- fit_data$distr == arg
  }
  nalogic <- is.na(fit_data$LLH)
  logic[nalogic] <- FALSE
  errorlogic <- fit_data$LLH < 0
  logic[errorlogic] <- FALSE
  newData <- all_fit_data[logic]
  returnVect <- rep(NA, sum(logic))
  for(i in c(1:sum(logic))){
    coefs <- NA
    coefs <- try(coef(newData[[i]])[[coeff]])
    returnVect[i] <- coefs
  }
  return(returnVect)
}
...
#final_all.analysis$coin <- as.factor(final_all.analysis$coin)
...
onlygarch %>% 
  select(name, distr, p, q, LLH, AIC, BIC) %>% 
  filter(name %in% c("eGARCH","TGARCH", "gjrGARCH")) %>% 
  pivot_wider(id_cols = c(distr, p, q), names_from = name, values_from = c(LLH, AIC, BIC)) %>% 
  View()
  mutate(tLLHgjr = as.numeric(LLH_TGARCH > LLH_gjrGARCH),
         eLLHT = as.numeric(LLH_eGARCH > LLH_TGARCH),
         eLLHgjr = as.numeric(LLH_eGARCH > LLH_gjrGARCH)) %>% 
  group_by(coin) %>% 
  summarize(mean1 = mean(tLLHgjr),
            mean2 = mean(eLLHT),
            mean3 = mean(eLLHgjr)) %>% 
  as.matrix()
...
igarch %>% 
  select(name, distr, p, q, coin, LLH) %>% 
  pivot_wider(names_from = c(distr, p, q, coin), values_from = c(LLH)) %>% 
  t() %>% 
  as.data.frame() %>% 
  #head()
  summarize(mean <- mean(V1 > V2))
...
final1 <- onlygarch %>% 
  filter(distr == c("std"))
percentage_total_full(final1, "name", "LLH")
percentage_total_full(final1, "name", "AIC")
percentage_total_full(final1, "name", "BIC")
...
for(coin.name in levels(final_all.analysis$coin)){
  temp <- final_all.analysis[final_all.analysis$coin == coin.name,]
  plot(percentage_total_full.coin(temp, "name", "LLH", coin.name))
  plot(percentage_total_full.coin(temp, "name", "AIC", coin.name))
  plot(percentage_total_full.coin(temp, "name", "BIC", coin.name))
}
...
coef.df <- read.xlsx(path2, "coef_df_final")
pval.df <- read.xlsx(path2, "pVal_df_final")
stdE.df <- read.xlsx(path2, "stdErr_df_final")
...
for(i in levels(all.analysis$distr)){
  print(i)
  temp <- head(coef.df[all.analysis$distr == i,c(24:26)], 1)
  print(3 - sum(is.na(temp)))
}


summary(coef.df)
for(i in levels(coef.df$name)){
  print(i)
  print(summary(coef.df[coef.df$name == i, c(1:26)]))
}
View(coef.df %>% 
       filter(omega < 0))
...
root <- sqrt(delta**2+(x-mu)**2)
expon <- exp(delta*gamma+beta*(x-mu))
K1 <- besselK((alpha*root), nu = 1, expon.scaled = FALSE)
nigdistr <- alpha*delta*K1 /pi/root * expon

xi <- delta*sqrt(alpha**2-beta**2)
rho = beta / alpha

head(coef.df[all.analysis$distr == "nig", c("pppshape", "skew")])
nig.dist <- ddist(distribution = "nig", x, mu = 0, sigma = 1, shape = xi, skew = rho)

plot(x, nig.dist, type = "l")
points(x, nigdistr, col = "red")


a = 50
x <- besselK(c(1:100), nu = a, expon.scaled = FALSE) / besselK(c(1:100), nu = a+1, expon.scaled = FALSE)
lines(x)


summary(coef.df[spec_dataframe_final$name=="iGARCH",])
